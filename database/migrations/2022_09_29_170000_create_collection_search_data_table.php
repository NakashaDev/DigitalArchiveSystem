<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateCollectionSearchDataTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('collection_search_data', function (Blueprint $table) {
            $table->integer('collection_id', true)->comment('資料ID');
            $table->smallInteger('collection_genre_id')->comment('資料分類ID');
            $table->string('manage_num', 64)->comment('管理番号');
            $table->string('title', 256)->comment('タイトル');
            $table->string('title_m', 256)->comment('タイトル（管理用）');
            $table->json('items')->comment('分類項目情報');
            $table->tinyInteger('is_public')->comment('公開・非公開');
            $table->integer('created_by')->comment('登録ユーザー');
            $table->timestamp('created_at')->useCurrent()->comment('登録日時');
            $table->timestamp('updated_at')->useCurrent()->comment('変更日時');

            $table->comment('資料検索用テーブル');
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('collection_search_data');
    }
}
